...
2차원 핵심정서를 구성한다. **두 축 모두 `QuantileTD` 에서 나온다.**

```
valence = 2·F̂_전역참조( r_obs + γ·V(s') ) − 1        수준(level)
arousal = 1 − exp( −W₁(Z_before(s,a), Z_after(s,a)) / κ )   오차(error)
λ_aff   = valence × arousal
```

**왜 두 축이 다른 대상을 읽는가 — level 대 error.**

| | 의미론 | 성질 | 필요한 참조 |
|---|---|---|---|
| valence | "지금 내 처지가 얼마나 나쁜가" | 지속적 | 현재 상황 **바깥** |
| arousal | "얼마나 예상 밖인가" | 자기소멸적 | 현재 상황 **자신** |

이상성의 주장은 "**지속되는** 나쁜 대우가 조절 자세를 옮긴다" 이지 "**놀라운**
나쁜 대우가" 가 아니다. 따라서 valence 는 수준이어야 하고, 수준 신호에는 (s,a)
바깥의 참조가 필요하다.

**valence 를 TD 오차로 두면 안 되는 이유(실측).** `Z(s,a)` 자신에 견주면 그것은
TD 오차이고, 만성 상호배신에서 +0.810 → +0.048 → −0.333 으로 진동하며 소멸한다.
더 나쁜 것은 학습 구간에서 **부호가 뒤집힌다**는 점이다 — 가치 추정이 낮은 곳에서
출발해 올라가므로 매 관측이 "예상보다 좋음" 이 되어, **만성 착취가 긍정 정서를
만든다.** 구성개념의 붕괴다.

**TD 목표를 쓰는 이유.** 즉각 보상만으로는 함의를 담지 못한다. 같은 보수라도
관계가 나아지는 중인지 무너지는 중인지가 다음 상태의 가치에 들어 있고,
`r + γ·V(s')` 가 그 둘을 하나로 요약한다.

**arousal 이 Z 를 읽는 이유.** 놀람의 대상은 보상이 아니라 **상황의 가치**다.
오랜 협력 관계에서 상대가 한 번 배신했을 때 즉각 보상 기대는 조금 바뀌지만
앞으로의 전망은 크게 무너진다 — Z 의 이동량만이 그 차이를 잡는다(실측: 충격
각성 배율 11.6배).

**`QuantileCode` 의 남은 역할.** 정서의 두 축이 모두 QRTD 로 옮겨가면서,
`QuantileCode` 는 전역 가치참조의 **컨테이너**로만 남는다 — 담기는 양이 보상에서
TD 목표로 바뀌었다. 참조는 구조적 지지범위 [r_min/(1−γ), r_max/(1−γ)] 로
초기화되고 social_lr=0.02 로만 이동한다.

### 2.2b `ipd/tom/self_policy.py` — SelfPolicy (형질공간 정책)

focal 은 행동 a_i 를 직접 고르지 않는다. **자기 형질** θ_i = (ρ, ω, η) 를 고르고,
행동은 그 형질의 우도에서 나온다.

```
P(a_i = C) = σ( β·( ρ·f + ω·g + η·f·g + s(λ, p_j) ) )      α ≡ 0, β ≡ 4
```

Albarracin 이 θ 를 도입한 이유가 지평에 따라 폭발하는 잠재상태를 ToM 으로 압축
하기 위함이었다면, **자기 정책도 같은 압축을 받아야 한다.** 자기만 행동수준에
남기면 자·타 표상이 비대칭해져 조망수용이 잘 정의되지 않는다. 계산상으로도
형질공간이 유리하다 — (f,g) 결합상태가 4개뿐이라 분기 없이 분포를 전방전파할 수
있어 후보당 O(4·H) 다.

**두 친사회 경로.** 친사회적 *행위*에는 두 동기가 있다.

| 경로 | 조절 대상 | 구동원 | 성격 |
|---|---|---|---|
| 공감 | λ | CoreAffect | 상대 후생을 직접 가치화 |
| 호혜 | ρ, ω, η | OpponentInversion | 상호작용 구조에 대한 조건부 반응 |

호혜는 **조건부** 친사회성이다(부정적 호혜=보복을 포함한다). 높은 ρ 가 실제로
협력을 늘리는지는 생태에 의존하는 **경험적 결과**이지 정의가 아니다.

α ≡ 0 은 단순화가 아니라 주장이다 — 무조건적 협력 성분을 없애 모든 협력이
호혜적이거나 공감적이도록 강제한다. 절편은 사라지지 않는다: s(λ,p) 가 λ 에 따라
−1.5 ~ +3.5 로 움직이므로 **λ 가 동적 절편**이 된다.

**형질 EFE.** G(θ_i) = −Σ γ^t[E[u_λ] + w_epi·IG_j] + w_cplx·KL(θ_i‖prior).
s(λ,p) = (1−λ)Δ_self + λΔ_other 임을 수치로 확인했으므로(전 조합 일치), 우도의
s 항이 이미 λ-가중 pragmatic 의 1-step 해석해다. rollout 은 그 다단계 확장이며,
EFE 안에서 (1−λ)self + λother 를 다시 합성하지 않는다. 상대의 인식항은 포함하지
않는다.

**복잡도 항이 필수적이다.** 없으면 증거 없이 형질이 표류한다 — ALLD 처럼 행동이
상수인 상대 앞에서는 f 가 −1 로 고정되어 ρ·f 가 사실상 절편이 되고, ρ 가 호혜성이
아니라 배신 장치로 선택된다(실측: 복잡도 항 없이 corr(ρ_self, ρ̂_j) = −0.22).
복잡도 항을 넣으면 corr = **+0.90** 이 된다.

**후보 생성 — SMC.** 원형 격자나 θ̂_j 근방 격자는 후보를 손으로 고르는 자유도다.
대신 자기 형질의 입자집합을 유지한다(제안 → 평가 → w ∝ exp(−γG) → 재표집).
3단계는 능동추론의 정책 사후 q(π) ∝ exp(−G(π)) 를 입자로 근사한 것이고,
재표집된 집합이 다음 라운드의 사전이 되므로 "사후를 사전으로" 가 자동이다.

### 2.2c `core/qrtd.py` — QRTD (분포적 가치·보상 학습)

`core.distributional.QuantileCode` 와 혼동하면 안 된다.

| | `QuantileCode` | `QuantileTD` / `RewardModel` |
|---|---|---|
| 대상 | 받아온 보상의 **주변분포** | **Z(s,a)** 수익 분포 / **R̂(joint)** 1-step 보상 |
| 색인 | 없음 | (상태, 행위) / joint outcome |
| 부트스트랩 | 없음 | 있음 (γ) |
| 역할 | valence 영점 · arousal (**정서**) | 행동 대안 간 기대효용 차이 (**행동**) |

**왜 필요한가.** `empathy_shift(λ,p)` 는 보수행렬을 알아야만 쓸 수 있다. 이전
판은 비정상 레짐에서도 HalloReg 에게만 매 라운드 현재 보수행렬을 넘겨주고
있었고, 고정전략은 그 정보를 받지 못했다. **H4 의 우위 중 얼마가 모형의 성질이고
얼마가 정보 접근 특권인지 분리할 수 없었다** — 이는 이론적 근거가 아니라 교란이다.

QRTD 는 관측된 보상만으로 R̂ 과 Z 를 학습해 그 교란을 제거한다.

| 조건 | 보수 접근 | 용도 |
|---|---|---|
| `--payoff-access oracle` | 행렬 직접 관측 | 현행, 상한 기준 |
| `--payoff-access naive` | 관측 보상만 (QRTD) | 공정 비교, H4 재검정 |

**이중계산 방지.** Z 는 수익이므로 그 차이에 미래가 이미 들어 있다. 따라서
지평 안은 R̂ 로 명시적으로 굴리고 **지평 너머만 Z 가 요약**한다.

    G(θ_i) = −E[ Σ_{t<H} u_t(R̂) + γ^H · Z(s_H) ] + w_cplx·KL

`empathy_shift` 는 폐기되지 않고 **아는 경우의 닫힌 해**로 남는다. ARCH V14 가
학습된 ŝ(λ,p) 의 해석해 수렴을 검사한다(최대 오차 0.065).

**위험민감 평가.** 분포를 학습하는 실익은 평균 아닌 통계량을 쓸 수 있다는 데
있다. `V̂_τ = mean{θ_i : τ_i ≤ τ}` (CVaR), 기본 τ=0.3. 이로써 `pessimism()` 이
표상에만 머물지 않고 행동으로 흐른다. τ 를 정서가 조절하게 하는 확장은 λ 의
삼중진입이 되므로 지금은 고정 상수로 둔다.
...
고정전략은 보수행렬을 읽지 않고 행동 이력에만 반응한다.

**이전 판의 교란과 그 교정.** 이전에는 "능동추론 에이전트는 EFE 의 선호를 현재
보수로 계산한다 — 이 비대칭이 H4 의 근거다" 라고 적었다. 그러나 이는 근거가 아니라
**교란**이다. HalloReg 만 보수행렬을 특권적으로 받고 고정전략은 못 받으므로,
비정상 조건에서 이기는 것이 당연하다. 지금까지 H4 가 반복 지지된 유일한 가설인데,
그 우위의 상당 부분이 모형의 우수성이 아니라 정보 접근 특권일 수 있었다.

`--payoff-access naive` 조건(§2.2c)이 이 교란을 제거한다. QRTD 로 보수를 관측에서
학습하면 고정전략과 정보 접근이 대등해지고, 그때도 우위가 남는지로 H4 를 재검정할
수 있다. ARCH V15 가 naive 조건에서 협력 관계가 붕괴하지 않음을 확인한다
(TFT 상대 보수 유지율 88.1%).
...
### 7.4 naive 기본 전환과 방문 빈도 편향 (미해결)

v1.5.2 에서 `--payoff-access` 기본값을 **naive** 로 전환했다. 환경의 보수 구조가
변동할 수 있으므로 에이전트가 보수행렬을 미리 관측하는 것은 정당화되지 않으며,
이전 기본값 oracle 은 고정전략과의 비교를 교란했다(§2.2c).

전환의 효과는 대체로 **개선**이다 (120R, 12시드):

| 상대 | 내 협력률 | CC율 | λ_final |
|---|---|---|---|
| ALLC | 0.943 | 0.903 | 0.791 |
| ALLD | 0.096 | 0.003 | 0.158 |

그러나 **호혜 경로가 무너진다.** `corr(ρ_self, ρ̂_j)` 가 oracle 의 +0.90 에서
naive 에서 **−0.55** 로 역전되고, ARCH V10·V11 이 실패한다.

원인은 R̂ 의 **방문 빈도 편향**이다. 협력이 정착하면 DC(배신-협력)를 거의 겪지
않아 그 칸이 초깃값에 머문다 — TFT 상대 실측에서 학습값 1.71 vs 참값 5.0.
rollout 의 효용이 왜곡되므로 호혜의 도구적 가치가 잘못 평가된다.

이는 naive 학습의 본질적 탐색 문제이며, 낙관적 초기화나 방문 횟수 기반 탐색
보너스가 필요하다. 임계를 낮춰 통과시키지 않고 실패로 기록한다. **현재 판의
호혜 경로 관련 주장(§2.2b 의 두 경로 해리)은 oracle 조건에서만 검증되었다.**

### 7.5 호혜 결합의 규모 의존성 (미해결)

`corr(ρ_self, ρ̂_j)` 는 라운드 수에 강하게 의존한다 (8시드 실측):

| 라운드 | 30 | 120 | 400 |
|---|---|---|---|
| corr | **−0.36** | **+0.81** | +0.26 |

30R 에서 **음수**인 것은 형질이 아직 사전 근처에 머물러 상대별로 갈리지 않았기
때문이고, 400R 에서 다시 낮아지는 것은 ALLD 에 장기 노출될 때 ρ 가 표류하기
때문이다(ρ_self 0.05 → 0.40). 두 현상 모두 미해결이다.

ARCH V10 은 ≥80R 에서 임계 0.5, 그 미만에서는 −0.1 을 적용하며, **스모크
규모에서는 실패로 기록된다.** 임계를 낮춰 통과시키지 않는다 — 이 지표가 규모에
따라 부호까지 바뀐다는 사실 자체가 보고되어야 할 결과다.

### 7.6 λ-단독 정책의 대가 (v2.0, 기록)

(ρ, ω, η) 를 행위 선택에서 폐기하면 호혜적 처벌 채널이 사라진다. 6시드 400R
정상상태 CC율 비교:

| 상대 | traits (v1.9) | λ-단독 (v2.0) |
|---|---|---|
| ALLC | 0.899 | 0.890 |
| TFT | 0.862 | **0.661** |
| WSLS | 0.632 | 0.646 |
| ALLD (협력률) | 0.351 | 0.359 |

TFT 가 크게 하락한다 — 배신당한 다음 라운드에 즉시 되갚는 반응이 A_x(s) 의
한 스텝 지연 학습으로 대체되기 때문이다. 방어와 WSLS 는 보상적 λ_sp 도입으로
회복되었으나(λ-단독 + 적합도-아핀 λ_sp 에서는 ALLD 협력률 0.487 이었다),
호혜자와의 협력 유지는 형질 경로가 담당하던 몫이 있었음이 확인된다.

기존 형질 경로는 `policy_mode="traits"` 로 보존되며, ARCH V10·V12 는 그 경로를
명시적으로 켜고 검사한다.

### 7.7 절편 규모 정규화의 되먹임 (해소됨, 기록)

v1.9.0 에서 절편을 형질 로짓 규모로 정규화할 때, 분모를 **현재 형질의 실측
규모(EMA)** 로 두면 되먹임이 생긴다 — 형질이 작아지면 분모가 줄어 절편이
커지고, 그러면 형질이 더 밀려나 다시 작아진다. 실측:

| 분모 | 120R corr | 400R corr | 400R ρ_self |
|---|---|---|---|
| 실측 규모(EMA) | +0.508 | **−0.424** | 전 유형 ≈0.1 로 수축 |
| **사전 규모(고정)** | **+0.606** | **+0.246** | 0.14~0.35 유지 |

사전 규모 `E|ρf+ωg+ηfg| ≈ sqrt(2/π)·sqrt(σ_ρ²+σ_ω²+σ_η²) = 1.046` 은 상수이며
"형질이 취할 수 있는 전형적 크기" 라는 뜻이라 되먹임이 없다.

### 7.8 인식항이 복원을 개선하지 않는다 (null)

형질 EFE 의 인식항은 기본 가중 w_epi=1.0 에서 pragmatic 대비 **2.2%** 에 불과해
사실상 무력하다(상태별 IG 최대차 0.285 nats vs pragmatic 누적 ~13). 두 항이
비슷해지는 w_epi=12 까지 올리면 형질 선택은 실제로 달라지지만(ρ_self 0.505 →
0.450), **상대 형질 복원 정확도는 개선되지 않았다**(|θ̂−θ*| 0.4044 vs 0.3997).
즉 "내생적 탐침" 은 기제로는 작동하나 더 나은 추론으로 이어지지 않는다.
ARCH V12 는 이 null 을 그대로 기록한다.

### 7.5 전역 가치참조의 장기 표류

valence 는 TD 목표를 SelfModel 의 전역 가치참조에 견준 **수준 신호**다. 그 참조는
학습되는 양(social_lr=0.02)이므로 아주 긴 단일 국면 노출에서는 결국 끌려간다 —
순수 착취를 300라운드 이어 주면 valence 가 −0.954 → **+0.042 로 부호가 역전**된다.
세션 규모(120라운드)에서는 유지되며(ALLD 상대 −0.555), 실험은 그 규모에서 수행
되지만, 지속 개체를 쓰는 집단 조건에서는 주의가 필요하다.

설정점이 적응한다는 것 자체는 이상성의 정의이지만 **부호 역전은 바람직하지 않다.**
사전 사회사를 소멸하지 않는 유사관측 앵커로 두는 것이 후보 해법이다.
